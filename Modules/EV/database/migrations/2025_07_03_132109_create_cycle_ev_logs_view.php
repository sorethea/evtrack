...
WITH ev_logs_base AS (
    ...
    GROUP BY l.id, l.cycle_id, l.vehicle_id, l.date, l.log_type,l.parent_id
),
...
charge_breakdown AS (
    SELECT
        ...
        SUM(CASE WHEN log_type != 'charging' AND prev_soc IS NOT NULL AND soc > prev_soc THEN soc - prev_soc ELSE 0 END) AS soc_increase_regen,
...
SELECT
	...
	CASE
	    WHEN `cr`.`next_root_soc` = 100 AND `lic`.`last_soc` >= 65 THEN
			65 + 35 * (((`cr`.`next_root_ac` - `lic`.`last_ac`) - ( `cr`.`next_root_ad` - `lic`.`last_ad` )) / ( `v`.`capacity` * ( `cr`.`next_root_soc` - `lic`.`last_soc` ) / 100 ))
		WHEN `cr`.`next_root_soc` = 100 AND `lic`.`last_soc` < 65 THEN
			65 + 35 * ((`cr`.`next_root_ac` - `lic`.`last_ac` - ( `cr`.`next_root_ad` - `lic`.`last_ad` ) - `v`.`capacity` * ( `cr`.`next_root_soc` - `lic`.`last_soc` - 35 ) / 100) / ( 35 * `v`.`capacity` / 100 ))
		ELSE 0
    END AS `next_soh`,
