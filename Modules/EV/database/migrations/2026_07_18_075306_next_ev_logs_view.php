    ...
    public function up(): void
    {
        DB::statement("
            ...
            SELECT
                ...
                b.id AS next_id,
                (SELECT SUM(ac) FROM ev_logs WHERE cycle_id = a.id AND a.log_type='charge') AS charge,
                (SELECT SUM(ac) FROM ev_logs WHERE cycle_id = a.id AND a.log_type<>'charge') AS regen
            FROM ev_logs a
            LEFT JOIN ev_logs b
                ON b.id = (
                    SELECT MIN(c.id)
